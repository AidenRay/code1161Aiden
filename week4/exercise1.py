...
def success_is_relative():
    ...
    # that it changes.
    # print(path, CWD)
    file_path = "week1/pySuccsessMessage.json" #or ../week1/etc
    """json_message = open(file_path)
    data = json.load(json_message)
    #data = data.read"""

    with open(file_path) as data_file:
        data = json.load(data_file) #or loads?""
    data = data["message"].strip()
    return data
...
def get_some_details():
    ...
    TIP: Keep a close eye on the format you get back. JSON is nested, so you
         ...
         dictionaries.
    """
    LOCAL = "C:/Users/aiden/Documents/CODE1161/code1161Aiden/week4"
    json_data = open(LOCAL + "/lazyduck.json").read()

    data = json.loads(json_data)
    lastName = data["results"][0]["name"]["last"].strip()
